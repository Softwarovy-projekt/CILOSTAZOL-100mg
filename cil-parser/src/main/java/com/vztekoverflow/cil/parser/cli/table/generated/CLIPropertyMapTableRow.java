...
public class CLIPropertyMapTableRow extends CLITableRow<CLIPropertyMapTableRow> {
	...
	public final CLITablePtr getParentTablePtr() {
		int offset = 0;
		final int rowNo;
		if (areSmallEnough(MAP_PARENT_TABLES)) {rowNo = getShort(offset)  & 0xFFFF;} else {rowNo = getInt(offset);}
	...
	public final CLITablePtr getPropertyListTablePtr() {
		...
		final int rowNo;
		if (areSmallEnough(MAP_PROPERTY_LIST_TABLES)) {rowNo = getShort(offset)  & 0xFFFF;} else {rowNo = getInt(offset);}
