...
public class CLIGenericParamConstraintTableRow extends CLITableRow<CLIGenericParamConstraintTableRow> {
	...
	public final CLITablePtr getOwnerTablePtr() {
		int offset = 0;
		final int rowNo;
		if (areSmallEnough(MAP_OWNER_TABLES)) {rowNo = getShort(offset)  & 0xFFFF;} else {rowNo = getInt(offset);}
	...
	public final CLITablePtr getConstraintTablePtr() {
		...
		return new CLITablePtr(MAP_CONSTRAINT_TABLES[codedValue & 3], (isSmall ? (0x0000ffff & codedValue) : codedValue) >>> 2);
	}
