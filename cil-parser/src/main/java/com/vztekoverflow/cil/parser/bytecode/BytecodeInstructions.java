...
import static com.vztekoverflow.cil.parser.bytecode.PrimitiveMarker.*;

import com.oracle.truffle.api.CompilerDirectives.CompilationFinal;

/**
 * Class holding instruction opcode constants and tables describing instruction parameters (like
 * length, stack effect, etc.).
 */
public class BytecodeInstructions {
  ...
  /**
   * An implementation of Table III.2: Binary Numeric Operations Stores the result type for A op B,
   * where op is add, div, mul, rem, or sub, for each possible combination of operand types.
   *
   * <p>Maps two {@code EVALUATION_STACK_TAG}s to an {@link PrimitiveMarker}. Invalid combinations
   * result in a null.
   */
  @CompilationFinal(dimensions = 2)
  public static final PrimitiveMarker[][] binaryNumericResultTypes =
      new PrimitiveMarker[EVALUATION_STACK_TAG_MAX + 1][EVALUATION_STACK_TAG_MAX + 1];
  /**
   * An implementation of Table III.5: Integer Operations Stores the result type for A op B, where
   * op is and, div.un, not, or, rem.un, xor, for each possible combination of operand types.
   *
   * <p>Maps two {@code EVALUATION_STACK_TAG}s to an {@link PrimitiveMarker}. Invalid combinations
   * result in a null.
   */
  @CompilationFinal(dimensions = 2)
  public static final PrimitiveMarker[][] binaryIntegerResultTypes =
      new PrimitiveMarker[EVALUATION_STACK_TAG_MAX + 1][EVALUATION_STACK_TAG_MAX + 1];
  /**
   * An array that maps from an opcode value to a {@link String} for the corresponding instruction
   * mnemonic.
   */
   ...
   *
   * <p>For dynamic instructions (like function calls), 0 should be used.
   */
  @CompilationFinal(dimensions = 1)
   ...
   */
  @CompilationFinal(dimensions = 1)
  private static final String[] prefixedNameArray = new String[256];
  /**
   * An array that maps from a prefixed opcode value to the length in bytes for the corresponding
   * instruction.
   */
  @CompilationFinal(dimensions = 1)
  private static final int[] prefixedLengthArray = new int[256];
  /**
   * An array that maps from a prefixed opcode value to the number of slots pushed on the stack by
   * the corresponding instruction.
   *
   * <p>For dynamic instructions (like function calls), 0 should be used.
   */
  @CompilationFinal(dimensions = 1)
  private static final int[] prefixedStackEffectArray = new int[256];
  ...
  static {
    binaryNumericResult(
        EVALUATION_STACK_TAG_INT32, EVALUATION_STACK_TAG_INT32, EVALUATION_STACK_INT32);
    binaryNumericResult(
        EVALUATION_STACK_TAG_INT32, EVALUATION_STACK_TAG_NATIVE_INT, EVALUATION_STACK_NATIVE_INT);
    binaryNumericResult(
        EVALUATION_STACK_TAG_INT64, EVALUATION_STACK_TAG_INT64, EVALUATION_STACK_INT64);
  ...
  private static void def(int opcode, String name, String format, int stackEffect) {
    assert nameArray[opcode] == null
        : "opcode " + opcode + " is already bound to name " + nameArray[opcode];
  ...
  /**
   * Defines an instruction for a prefixed opcode by entering recording its name, length and
   * stackEffect.
   *
   * @param opcode the opcode to define
   * @param name mnemonic of the instruction
  ...
  private static void defPrefixed(int opcode, String name, String format, int stackEffect) {
    opcode = opcode & 0xFF;
    assert prefixedNameArray[opcode] == null
        : "opcode " + opcode + " is already bound to name " + prefixedNameArray[opcode];
  ...
  /**
   * Get length of the instruction for the specified opcode. Supports prefixed opcodes.
   *
  ...
  /**
   * Get the mnemonic name of the instruction for the specified opcode. Supports prefixed opcodes.
   *
   * @param opcode the instruction opcode
  ...
  /**
   * Get the stack effect of the instruction for the specified opcode. Supports prefixed opcodes.
   *
   * @param opcode the instruction opcode
  ...
  /**
   * Defines a valid combination in the Binary Numeric Operations table. Makes sure all combinations
   * are commutative.
   *
   ...
   * @param result an {@link PrimitiveMarker} for the resulting evaluation stack type
   */
  public static void binaryNumericResult(byte arg1, byte arg2, PrimitiveMarker result) {
  ...
  /**
   * Defines a valid combination in the Integer Operations table. Makes sure all combinations are
   * commutative.
   *
   * @param arg1 an {@code EVALUATION_STACK_TAG} representing the first operand
   * @param arg2 an {@code EVALUATION_STACK_TAG} representing the second operand
   * @param result an {@link PrimitiveMarker} for the resulting evaluation stack type
   */
  public static void binaryIntegerResult(byte arg1, byte arg2, PrimitiveMarker result) {
