...
public class CLICustomAttributeTableRow extends CLITableRow<CLICustomAttributeTableRow> {
	...
	public final CLITablePtr getParentTablePtr() {
		...
		return new CLITablePtr(MAP_PARENT_TABLES[codedValue & 31], (isSmall ? (0x0000ffff & codedValue) : codedValue) >>> 5);
	}
	...
	public final CLITablePtr getTypeTablePtr() {
		...
		if (isSmall) {codedValue = getShort(offset) & 0xFFFF; } else {codedValue = getInt(offset);}
		if((isSmall && (codedValue & 0xffff) == 0xffff) || (!isSmall && (codedValue & 0xffffffff) == 0xffffffff)) return null;
		return new CLITablePtr(MAP_TYPE_TABLES[codedValue & 3], (isSmall ? (0x0000ffff & codedValue) : codedValue) >>> 2);
	}
