...
public class CLITableClassesGenerator {
  ...
  private static final String PACKAGE = "com.vztekoverflow.cil.parser.cli.table.generated";
  ...
  public static void generatePenaltiesCode(HeapPenalties penalties, PrintWriter writer) {
    ...
    if (penalties.stringHeapPenalty != 0) {
      writer.println(
          "\t\tif (tables.isStringHeapBig()) offset += " + penalties.stringHeapPenalty + ";");
    }

    if (penalties.GUIDHeapPenalty != 0) {
      writer.println(
          "\t\tif (tables.isGUIDHeapBig()) offset += " + penalties.GUIDHeapPenalty + ";");
    }

    if (penalties.blobHeapPenalty != 0) {
      writer.println(
          "\t\tif (tables.isBlobHeapBig()) offset += " + penalties.blobHeapPenalty + ";");
    }
  ...
  public static void generateConstantsClass(ArrayList<TableDefinition> tables, String outputDir)
    ...
    try (PrintWriter writer = new PrintWriter(path, "UTF-8")) {
      ...
      for (TableDefinition tableDefinition : tables) {
        writer.println(
            String.format(
                "\tpublic static final byte CLI_TABLE%s = %d;",
                nameToConstName(tableDefinition.name), tableDefinition.id));
      }

      writer.println(
          String.format(
              "\tpublic static final byte CLI_TABLE_MAX_ID = %d;",
              tables.get(tables.size() - 1).id));
  ...
  public static void generateHeadsClass(ArrayList<TableDefinition> tables, String outputDir)
      throws FileNotFoundException, UnsupportedEncodingException {
    String path = outputDir + "/CLITableHeads.java";
    try (PrintWriter writer = new PrintWriter(path, "UTF-8")) {
      writer.println("package " + PACKAGE + ";");
      writer.println();
      writer.println("import com.vztekoverflow.cil.parser.cli.table.CLITableRow;");
      writer.println("import com.vztekoverflow.cil.parser.cli.table.CLITables;");
      writer.println();
      ...
      for (TableDefinition table : tables) {
        ...
        writer.println(
            String.format(
                "\tprivate final %s %s;", className, decapitalize(table.name) + "TableHead"));
      }
      ...
      for (TableDefinition table : tables) {
        ...
        writer.println(
            String.format(
                "\t\t%s = new %s(tables, cursor, 0);",
                decapitalize(table.name) + "TableHead", className));
        writer.println(
            String.format(
                "\t\tcursor += getTableLength(tables, %s);",
                decapitalize(table.name) + "TableHead"));
      }
      writer.println("\t}");
      ...
      for (TableDefinition table : tables) {
        ...
        writer.println(
            String.format(
                ...
                className, table.name + "TableHead", decapitalize(table.name) + "TableHead"));
      }
  ...
  public static void generateGetColumn(
      HeapPenalties penalties, String fieldName, String fieldType, PrintWriter writer) {
      ...
      case 'h':
        {
          ...
          writer.println(
              String.format("\tpublic final CLI%sHeapPtr get%sHeapPtr() {", streamName, fieldName));
          generatePenaltiesCode(penalties, writer);
          writer.println("\t\tint heapOffset=0;");
          writer.println(
              String.format(
                  "\t\tif (tables.is%sHeapBig()) { heapOffset = getInt(offset); } else { heapOffset = getUShort(offset); }",
                  streamName));
      ...
      case 'i':
        {
          if (!fieldType.contains("|")) {
            String tableConstant =
                "CLITableConstants.CLI_TABLE" + nameToConstName(fieldType.substring(1));

            writer.println("\t@CompilerDirectives.CompilationFinal(dimensions = 1)");
            writer.println(
                String.format(
                    "\tprivate static final byte[] MAP%s_TABLES = new byte[] {%s};",
                    nameToConstName(fieldName), tableConstant));

            writer.println(
                String.format("\tpublic final CLITablePtr get%sTablePtr() { ", fieldName));
            ...
            writer.print(
                String.format(
                    "\tprivate static final byte[] MAP%s_TABLES = new byte[] { ",
                    nameToConstName(fieldName)));
                    ...
                    .map(
                        s ->
                            s.equals("_")
                                ...
                                : "CLITableConstants.CLI_TABLE" + nameToConstName(s))
  ...
  public static void generateTableClass(TableDefinition table, String outputDir)
      throws FileNotFoundException, UnsupportedEncodingException {
    String className = "CLI" + table.name + "TableRow";
    ...
    try (PrintWriter writer = new PrintWriter(path, "UTF-8")) {
      ...
      writer.println("import com.vztekoverflow.cil.parser.cli.table.*;");
      writer.println(
          String.format("public class %s extends CLITableRow<%s> {", className, className));
      writer.println("");

      // constructor
      writer.println(
          String.format("\tpublic %s(CLITables tables, int cursor, int rowIndex) {", className));
