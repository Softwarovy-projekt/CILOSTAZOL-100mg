...
public class CLIMethodSemanticsTableRow extends CLITableRow<CLIMethodSemanticsTableRow> {
	...
	public final CLITablePtr getMethodTablePtr() {
		int offset = 2;
		final int rowNo;
		if (areSmallEnough(MAP_METHOD_TABLES)) {rowNo = getShort(offset)  & 0xFFFF;} else {rowNo = getInt(offset);}
	...
	public final CLITablePtr getAssociationTablePtr() {
		...
		return new CLITablePtr(MAP_ASSOCIATION_TABLES[codedValue & 1], (isSmall ? (0x0000ffff & codedValue) : codedValue) >>> 1);
	}
