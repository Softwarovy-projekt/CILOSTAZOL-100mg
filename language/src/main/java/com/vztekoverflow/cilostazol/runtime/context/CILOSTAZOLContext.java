...
public class CILOSTAZOLContext {
  ...
  private final Map<ArrayCacheKey, ArrayTypeSymbol> arrayCache =
          new HashMap<>();
  ...
  public ArrayTypeSymbol resolveArray(TypeSymbol elemType, int rank) {
    ...
    return arrayCache.computeIfAbsent(
            cacheKey,
            k -> ArrayTypeSymbol.ArrayTypeSymbolFactory.create(k.elemType(), k.rank(), elemType.getDefiningModule()));
  }
