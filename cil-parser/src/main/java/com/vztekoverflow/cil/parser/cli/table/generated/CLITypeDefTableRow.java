...
public class CLITypeDefTableRow extends CLITableRow<CLITypeDefTableRow> {
	...
	public final CLITablePtr getExtendsTablePtr() {
		...
		return new CLITablePtr(MAP_EXTENDS_TABLES[codedValue & 3], (isSmall ? (0x0000ffff & codedValue) : codedValue) >>> 2);
	}
	...
	public final CLITablePtr getFieldListTablePtr() {
		...
		final int rowNo;
		if (areSmallEnough(MAP_FIELD_LIST_TABLES)) {rowNo = getShort(offset)  & 0xFFFF;} else {rowNo = getInt(offset);}
	...
	public final CLITablePtr getMethodListTablePtr() {
		...
		final int rowNo;
		if (areSmallEnough(MAP_METHOD_LIST_TABLES)) {rowNo = getShort(offset)  & 0xFFFF;} else {rowNo = getInt(offset);}
