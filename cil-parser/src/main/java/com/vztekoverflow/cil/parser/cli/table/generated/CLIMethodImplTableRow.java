...
public class CLIMethodImplTableRow extends CLITableRow<CLIMethodImplTableRow> {
	...
	public final CLITablePtr getKlassTablePtr() {
		int offset = 0;
		final int rowNo;
		if (areSmallEnough(MAP_KLASS_TABLES)) {rowNo = getShort(offset)  & 0xFFFF;} else {rowNo = getInt(offset);}
	...
	public final CLITablePtr getMethodBodyTablePtr() {
		...
		return new CLITablePtr(MAP_METHOD_BODY_TABLES[codedValue & 1], (isSmall ? (0x0000ffff & codedValue) : codedValue) >>> 1);
	}
	...
	public final CLITablePtr getMethodDeclarationTablePtr() {
		...
		if (isSmall) {codedValue = getShort(offset) & 0xFFFF; } else {codedValue = getInt(offset);}
		if((isSmall && (codedValue & 0xffff) == 0xffff) || (!isSmall && (codedValue & 0xffffffff) == 0xffffffff)) return null;
		return new CLITablePtr(MAP_METHOD_DECLARATION_TABLES[codedValue & 1], (isSmall ? (0x0000ffff & codedValue) : codedValue) >>> 1);
	}
