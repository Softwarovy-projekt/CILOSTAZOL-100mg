...
public class CILMethodNode extends CILNodeBase implements BytecodeOSRNode {
  ...
  private Object execute(VirtualFrame frame, int pc, int topStack) {
    ...
    while (true) {
      ...
      switch (curOpcode) {
        ...
        case LDELEM:
          CILOSTAZOLFrame.putObject(frame, topStack - 2, (StaticObject) ((StaticObject)getJavaArrElem(frame, topStack - 1)).clone());
          break;
        case LDELEM_REF:
          CILOSTAZOLFrame.putObject(frame, topStack - 2, (StaticObject)getJavaArrElem(frame, topStack - 1));
          break;
        ...
        case LDELEM_I2, LDELEM_U2:
          CILOSTAZOLFrame.putInt32(frame, topStack - 2, (short)getJavaArrElem(frame, topStack - 1));
          break;
        case LDELEM_I8:
          CILOSTAZOLFrame.putInt64(frame, topStack - 2, (long)getJavaArrElem(frame, topStack - 1));
          break;
        case LDELEM_R4:
          CILOSTAZOLFrame.putNativeFloat(frame, topStack - 2, (float)getJavaArrElem(frame, topStack - 1));
          break;
        case LDELEM_R8:
          CILOSTAZOLFrame.putNativeFloat(frame, topStack - 2, (double)getJavaArrElem(frame, topStack - 1));
          break;
        case LDELEMA:
          CILOSTAZOLFrame.putObject(frame, topStack - 2,getMethod().getContext().getAllocator().createArrayElementReference(
                  SymbolResolver.resolveReference(ReferenceSymbol.ReferenceType.ArrayElement, getMethod().getContext()),
  ...
  {
    TypeSymbol elemType = SymbolResolver.resolveType(token, getMethod().getTypeArguments(), getMethod().getDefiningType().getTypeArguments(), getMethod().getModule());
    ...
    Object value = switch (elemType.getSystemType()) {
